...
strings! {
    ...
    postman_key_help_op => "PaperBoy asks 1Password for the key each time it runs, so no key is kept here. Needs 1Password's op command.", "PaperBoy demande la clé à 1Password à chaque exécution ; aucune clé n'est conservée ici. Nécessite la commande op de 1Password.", "PaperBoy henter nøglen fra 1Password ved hver kørsel, så ingen nøgle gemmes her. Kræver 1Passwords op-kommando.";
    postman_key_help_ssm => "PaperBoy reads the key from AWS Parameter Store each time it runs, so no key is kept here. Needs the AWS CLI.", "PaperBoy lit la clé dans AWS Parameter Store à chaque exécution ; aucune clé n'est conservée ici. Nécessite l'AWS CLI.", "PaperBoy læser nøglen fra AWS Parameter Store ved hver kørsel, så ingen nøgle gemmes her. Kræver AWS CLI'en.";
