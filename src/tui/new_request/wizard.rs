...
/// One editable `[Reports]` row: `name: <hurl query>`, identical in shape to a
/// [`CaptureRow`] (it reuses [`CapCol`] for its two columns). Where a capture
/// binds a variable for later requests, a report field's query is evaluated
/// against the response purely to pull a value into the generated report. A
/// request with no `[Reports]` rows contributes its whole response instead.
...
pub(crate) enum NewField {
    ...
    /// The "+ Add …" row of one of the three KVD sections.
    AddKvd(KvdKind),
    ...
    AddCapture,
    /// A focused cell of a `[Reports]` row (name or expression column).
    Report(usize, CapCol),
...
pub(crate) struct NewReq {
    ...
    /// Set during draw: whether the Form table's Description column currently
    /// has room. Focus navigation skips it when it does not.
    ...
    /// than fit on screen. The Headers/Cookies/Queries scroll offsets live on
    /// their [`KvdSection`]s instead.
...
impl NewReq {
    ...
    pub(crate) fn active_editor(&mut self) -> Option<&mut Editor> {
        match self.focus {
            ...
            NewField::Kvd(KvdKind::Header, i, col) => {
                self.headers.get_mut(i).and_then(|r| r.cell_mut(col))
            }
            NewField::Kvd(KvdKind::Cookie, i, col) => {
                self.cookies.get_mut(i).and_then(|r| r.cell_mut(col))
            }
    ...
    /// True when every `[Reports]` row is blank — the section is then skipped
    ...
    /// Like [`Self::assert_entry`], but for `[Reports]`.
    pub(crate) fn report_entry(&self) -> NewField {
    ...
    /// Delete the currently focused Header/Cookie/Query/Form/Assert/Capture
    /// row, moving focus to the row that slides into its place (same column),
    /// or to the section's "+ Add …" row when it becomes empty. A no-op when
    /// focus isn't on a deletable row.
    pub(crate) fn delete_focused_row(&mut self) {
        self.focus = match self.focus {
            NewField::Kvd(kind, i, _) if i < self.kvd(kind).len() => {
                self.kvd_mut(kind).remove(i);
                if self.kvd(kind).is_empty() {
                    kind.add_field()
                } else {
                    kind.field(i.min(self.kvd(kind).len() - 1), HdrCol::Key)
                }
            }
            ...
            NewField::Report(i, _) if i < self.reports.len() => {
    ...
    /// Toggle the enabled flag of the focused Header/Cookie/Query/Form row in
    /// place, leaving focus untouched so it can be pressed mid-edit. A no-op on
    /// fields without an enabled flag (everything else, including Asserts/
    /// Captures).
    pub(crate) fn toggle_focused_enabled(&mut self) {
        let row = match self.focus {
            NewField::Kvd(kind, i, _) => self.kvd_mut(kind).get_mut(i).map(|r| &mut r.enabled),
    ...
    /// Like [`Self::kvd_entry`], but for the Form/Multipart section.
    ...
    /// Like [`Self::up_into_kvd`], but for leaving the first Capture row
    ...
    /// Like [`Self::up_into_asserts`], but for leaving the first Report row
    /// upward into Captures: lands on the last Capture row, or "+ Add Capture"
    /// when Captures is empty.
    pub(crate) fn up_into_captures(&self) -> NewField {
        if self.captures.is_empty() {
            NewField::AddCapture
        } else {
    ...
    /// The visually-navigable columns of Form row `i`. Starts from the
    /// width-based `form_row_cells`, then drops the one cell that is truly
    /// dead for the row's kind: a `Base64File` row's Content-Type is a
    /// non-editable `—` placeholder (the blob is sent as plain text), so
    /// arrow keys skip it. The Base64 Prefix stays navigable on every kind
    /// (it's inert but still editable on non-Base64 rows), matching the way
    /// it's drawn.
    fn form_row_cells_for(&self, i: usize) -> Vec<FormCol> {
        let mut cells = self.form_row_cells();
        if self.form_fields.get(i).map(|r| r.kind) == Some(FormFieldKind::Base64File) {
            cells.retain(|c| *c != FormCol::Ctype);
        }
        cells
    }

    /// The column to the left of `col` within Form row `i`, if any (arrow-key
    /// navigation — includes the Enabled checkbox, unlike the Tab order, and
    /// skips the dead Content-Type cell on a `Base64File` row).
    pub(crate) fn prev_form_col(&self, i: usize, col: FormCol) -> Option<FormCol> {
        let cells = self.form_row_cells_for(i);
        let idx = cells.iter().position(|c| *c == col)?;
        idx.checked_sub(1).map(|p| cells[p])
    }

    /// The column to the right of `col` within Form row `i`, if any (arrow-key
    /// navigation).
    pub(crate) fn next_form_col(&self, i: usize, col: FormCol) -> Option<FormCol> {
        let cells = self.form_row_cells_for(i);
        let idx = cells.iter().position(|c| *c == col)?;
    ...
    fn tab_stops(&self) -> Vec<NewField> {
        ...
        for kind in KvdKind::ALL {
            if self.kvd_blank(kind) {
                ...
                let cells = self.kvd(kind).tab_cells();
                for i in 0..self.kvd(kind).len() {
                    v.extend(cells.iter().map(|&c| kind.field(i, c)));
                }
                v.push(kind.add_field());
            }
        }
        ...
        if self.reports_blank() {
            v.push(self.report_entry());
        } else {
            for i in 0..self.reports.len() {
                v.push(NewField::Report(i, CapCol::Name));
                v.push(NewField::Report(i, CapCol::Expr));
            }
...
pub(crate) fn draw_new_request(
    ...
    let on_toggleable_row = matches!(
        form.focus,
        NewField::Kvd(KvdKind::Header, i, _) if i < form.headers.len()
    ) || matches!(
        form.focus,
        NewField::Kvd(KvdKind::Cookie, i, _) if i < form.cookies.len()
    ) || matches!(
        form.focus,
...
/// Draw one KVD section's label + table (Headers, Cookies or Queries) into
/// the given (label, table) rects. The `kind` picks which section's rows,
/// title and focus highlighting to use.
fn draw_kvd_section(
    f: &mut Frame,
    label: Rect,
    table: Rect,
...
/// Draw one KVD section's table (Headers, Cookies or Queries). All three share
/// the identical `Enabled/Key/Value/Description` column layout; only Headers
...
pub(crate) fn draw_kvd_table(
    ...
    let focused_idx = match form.focus {
        NewField::Kvd(k, i, _) if k == kind => Some(i),
        _ => None,
    };
    // Only Headers has the suggestion dropdown anchored under its Key cell.
    let key_cell_rect = (kind == KvdKind::Header).then_some(&form.key_cell_rect);
...
pub(crate) fn draw_report_table(f: &mut Frame, area: Rect, form: &NewReq, s: &Strings, th: &Theme) {
    ...
    if let Some(hrect) = header_rect {
        let hcells = cell_rects(hrect);
        f.render_widget(lbl(s.report_name), hcells[0]);
        f.render_widget(lbl(s.report_expr), hcells[1]);
    }

    for (slot, row_area) in data_rects.iter().enumerate() {
        let i = start + slot;
        let row = &form.reports[i];
        let cells = cell_rects(*row_area);
        draw_header_cell(
            f,
            cells[0],
    ...
    let add_focused = form.focus == NewField::AddReport;
    f.render_widget(
        Paragraph::new(Span::styled(
            s.add_report.to_string(),
            Style::default()
                .fg(if add_focused { th.accent } else { th.dim })
                .add_modifier(Modifier::BOLD),
        )),
