...
pub(crate) fn ordered(a: TextPos, b: TextPos) -> (TextPos, TextPos) {
    if a <= b { (a, b) } else { (b, a) }
}
...
pub(crate) fn strip_positions(text: &str, exclude: &std::collections::HashSet<TextPos>) -> String {
    if exclude.is_empty() {
